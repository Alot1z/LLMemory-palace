...
import type { CodeIndex, IndexedFile, Symbol, ExtractedPattern } from '../types.js';
...
const DEFAULT_CONFIG: BinaryFormatConfig = {
...
// Internal index structure for binary format
...
export class BinaryFormat {
  private config: BinaryFormatConfig;
  private hashUtil: SemanticHash;
  ...
  // Encoding
  async encode(index: CodeIndex): Promise<Buffer> {
    this.reset();
  ...
  private convertToBinaryIndex(index: CodeIndex): BinaryIndex {
    const files = new Map<string, BinaryFileInfo>();
    
    for (const file of index.files) {
      const symbols: BinarySymbolInfo[] = file.analysis.symbols.map(s => ({
        name: s.name,
        kind: s.type,
        hash: file.hash,
  ...
  private buildStringPool(index: BinaryIndex): void {
    let id = 0;
  ...
  private writeFileIndex(index: BinaryIndex): void {
  ...
  private writeSymbolTable(index: BinaryIndex): void {
    ...
    for (const file of index.files.values()) {
      for (const symbol of file.symbols) {
        ...
        this.writeByte(this.symbolKindToByte(symbol.kind as BinarySymbolInfo['kind']));
  ...
  private writePatternTable(index: BinaryIndex): void {
  ...
  private writeSourceData(_index: BinaryIndex): void {
  ...
  private writeMetadata(index: BinaryIndex): void {
  ...
  // Decoding
  async decode(buffer: Buffer): Promise<BinaryIndex> {
    this.buffer = buffer;
    this.offset = 0;
  ...
  private readHeader(): BinaryHeader {
    return {
  ...
  private readFileIndex(): BinaryIndex {
    const count = this.readUInt32();
  ...
  private readSymbolTable(index: BinaryIndex): void {
    const count = this.readUInt32();
  ...
  private readPatternTable(index: BinaryIndex): void {
    const count = this.readUInt32();
  ...
  private readMetadata(index: BinaryIndex): void {
    const count = this.readUInt32();
  ...
  private writeStringRef(value: string): void {
  ...
  private writeChunkHeader(type: ChunkType, length: number): void {
    this.writeByte(type);
    this.writeUInt32(length);
    this.writeUInt32(0); // checksum placeholder
  }

  private updateChunkLength(): void {
  ...
  private symbolKindToByte(kind: BinarySymbolInfo['kind']): number {
  ...
  private byteToSymbolKind(byte: number): BinarySymbolInfo['kind'] {
    const kinds: Record<number, BinarySymbolInfo['kind']> = {
      1: 'function',
      2: 'class',
      3: 'interface',
      4: 'type',
  ...
  // Utility methods
  getFormatInfo(): { version: number; features: string[] } {
    return {
...
export function createBinaryFormat(config?: Partial<BinaryFormatConfig>): BinaryFormat {
