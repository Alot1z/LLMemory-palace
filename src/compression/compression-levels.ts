...
export interface CompressionLevelConfig {
  ...
  expectedRatio: [number, number]; // [min, max] compression ratio
  useCases: string[];
  performance: {
...
export interface LevelSelectionCriteria {
  dataSize?: number;
  uniqueRatio?: number; // Ratio of unique characters
 ...
 */
export function getCompressionLevel(level: CompressionLevel): CompressionLevelConfig {
 ...
 */
export function getAllCompressionLevels(): CompressionLevelConfig[] {
 ...
 */
export function getCompressionLevelNames(): Record<CompressionLevel, string> {
  return {
 ...
 */
export function parseCompressionLevel(value: string | number | undefined): CompressionLevel {
  if (value === undefined) {
    return 3; // Default
  }
  ...
  if (typeof value === 'number') {
    if (value >= 1 && value <= 4) {
      return Math.floor(value) as CompressionLevel;
    }
  ...
  if (!isNaN(numLevel) && numLevel >= 1 && numLevel <= 4) {
    return numLevel as CompressionLevel;
  }
  ...
  for (const [level, config] of Object.entries(COMPRESSION_LEVELS)) {
    if (config.name === normalized || 
        ...
        config.name.replace('_', ' ') === normalized) {
      return parseInt(level) as CompressionLevel;
    }
  }
 ...
 */
export function selectOptimalLevel(criteria: LevelSelectionCriteria): CompressionLevel {
  ...
  if (criteria.targetRatio) {
    if (criteria.targetRatio >= 500) return 4;
    if (criteria.targetRatio >= 200) return 3;
    if (criteria.targetRatio >= 50) return 2;
    return 1;
  }
 ...
 */
export function getTechniquesForLevel(level: CompressionLevel): CompressionTechnique[] {
 ...
 */
export function hasTechnique(level: CompressionLevel, techniqueName: string): boolean {
  return COMPRESSION_LEVELS[level].techniques.some(
 ...
 */
export function getExpectedRatio(level: CompressionLevel): [number, number] {
 ...
 */
export function getRecommendedLevel(useCase: string): CompressionLevel {
 ...
 */
export function isValidCompressionLevel(level: unknown): level is CompressionLevel {
 ...
 */
export function createLevelSummary(level: CompressionLevel): string {
