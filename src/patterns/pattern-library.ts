...
export class PatternLibrary {
  ...
  findPatternInstances(content: string, template: string): FoundPatternInstance[] {
    const instances: FoundPatternInstance[] = [];
   ...
   */
  findForModule(moduleName: string): (Pattern & { name: string })[] {
    const patterns: (Pattern & { name: string })[] = [];
    
    for (const [name, pattern] of this.patterns) {
      // Check if pattern is relevant to module
      if (pattern.instances.some(i => 
        (i.entity as string)?.toLowerCase() === moduleName.toLowerCase() ||
        (i.name as string)?.toLowerCase() === moduleName.toLowerCase()
      )) {
